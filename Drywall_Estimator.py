...
# with optional exclusion of windows and doors per room. Outputs SQFT & SQM, per-room breakdown,
# and CSV download. Ready to be merged later into your insulation app as another tab.
...
with st.sidebar:
    ...
    waste_pct = st.number_input("Waste %", min_value=0.0, max_value=50.0, value=10.0, step=0.5, help="Applied to wall+ceiling net areas") if include_waste else 0.0
...
for i in range(int(room_count)):
    ...
    with st.container(border=True):
        ...
        with c4:
            height = st.number_input(f"Wall height (ft) #{i+1}", min_value=0.0, step=0.1, value=default_wall_h, key=f"h_{i}")

        c5, c6, c7 = st.columns([1,1,1])
        with c5:
            include_ceiling = st.checkbox(f"Include ceiling? #{i+1}", value=True, key=f"ceil_inc_{i}")
        with c6:
            num_windows = st.number_input(f"Windows (count) #{i+1}", min_value=0, max_value=20, value=0, step=1, key=f"w_count_{i}")
        with c7:
            num_doors = st.number_input(f"Doors (count) #{i+1}", min_value=0, max_value=20, value=0, step=1, key=f"d_count_{i}")
        ...
        if num_windows > 0:
            ...
            for w in range(num_windows):
                ...
                with wc1:
                    w_w = st.number_input(f"Window {w+1} width (ft) [R{i+1}]", min_value=0.0, step=0.1, key=f"win_w_{i}_{w}")
                with wc2:
                    w_h = st.number_input(f"Window {w+1} height (ft) [R{i+1}]", min_value=0.0, step=0.1, key=f"win_h_{i}_{w}")
        ...
        if num_doors > 0:
            ...
            for d in range(num_doors):
                ...
                with dc1:
                    d_w = st.number_input(f"Door {d+1} width (ft) [R{i+1}]", min_value=0.0, step=0.1, key=f"door_w_{i}_{d}")
                with dc2:
                    d_h = st.number_input(f"Door {d+1} height (ft) [R{i+1}]", min_value=0.0, step=0.1, key=f"door_h_{i}_{d}")
        ...
        if show_intermediate:
            st.caption(
                f"Perimeter: {perimeter:.2f} ft | Wall gross: {wall_area_gross:.2f} ft² | Openings: {openings_area:.2f} ft² | Net walls: {wall_area_net:.2f} ft² | Ceiling: {ceiling_area:.2f} ft²"
            )
...
if rooms_data:
    ...
    show_cols = [
        "room","length_ft","width_ft","height_ft","wall_area_net_ft2","ceiling_area_ft2","total_area_ft2","total_area_m2","total_with_waste_ft2","total_with_waste_m2"
    ]
    nice_names = {
        "room":"Room",
        "length_ft":"Length (ft)",
    ...
    # Totals
    total_ft2 = df["total_area_ft2"].sum()
    total_m2 = total_ft2 * FT2_TO_M2
    total_waste_ft2 = df["total_with_waste_ft2"].sum()
    ...
    for _, r in df.iterrows():
        lines.append(
            f"- {r['room']}: Walls {r['wall_area_net_ft2']:.2f} ft², Ceiling {r['ceiling_area_ft2']:.2f} ft², Total {r['total_area_ft2']:.2f} ft² ({r['total_area_ft2']*FT2_TO_M2:.2f} m²)"
        )
    lines.append("")
    lines.append(f"Grand Total: {total_ft2:.2f} ft² ({total_m2:.2f} m²)")
    if include_waste:
        lines.append(f"Grand Total w/ {waste_pct:.1f}% waste: {total_waste_ft2:.2f} ft² ({total_waste_m2:.2f} m²)")
    txt = "\n".join(lines)
